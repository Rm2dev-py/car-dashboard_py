// src/components/Cligno_G_D.tsx
import { useEffect, useState, useRef } from "react";
import { useWS } from "../WebSocketProvider";

export default function Cligno_G_D() {
  const wsData = useWS();
  const isActive = wsData?.voyant_cligno_g === 1 || wsData?.voyant_cligno_g === true;
  const [visible, setVisible] = useState(false);
  const [position, setPosition] = useState(() => {
    const saved = localStorage.getItem("cligno_g_position");
  ...
  useEffect(() => {
    localStorage.setItem("cligno_g_position", JSON.stringify(position));
  }, [position]);
  ...
  return (
    <div
      ...
      style={{
        ...
        cursor: "grab",
        touchAction: "none",
        backgroundColor: "transparent", // Pour débogage : rgba(0,255,0,0.1)
      }}
    ...
    >
      ...
      >
        <mask id="path-1-outside-1_1144_901" maskUnits="userSpaceOnUse" x="717.457" y="4.85025" width="46.25" height="39.72" fill="black">
          <rect fill="white" x="717.457" y="4.85025" width="46.25" height="39.72" />
          <path
